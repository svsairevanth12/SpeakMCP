...
function testNoOpDetection(response, toolCapabilities, noOpCount) {
  ...
  if (!hasToolCalls) {
    ...
    const shouldNudge = newNoOpCount >= 2 || (isActionableRequest && newNoOpCount >= 1);
...
const noOpTests = [
  ...
  },
  {
    ...
    response: { content: "Executing tool", toolCalls: [{ name: "test", arguments: {} }] },
    noOpCount: 1,
    expectedNudge: false
  }
];
...
noOpTests.forEach((test, index) => {
  const result = testNoOpDetection(test.response, mockToolCapabilities, test.noOpCount);
...
console.log("✅ 1. Fixed LLMToolCallResponse interface to include needsMoreWork");
