...
async function fetchOpenAIModels(baseUrl?: string, apiKey?: string): Promise<ModelInfo[]> {
  ...
  if (isOpenRouter) {
    ...
    filteredModels = data.data.filter(model =>
  ...
  return filteredModels
    ...
    .sort((a, b) => {
      if (isOpenRouter) {
        ...
        const priorityDiff = getOpenRouterPriority(a.id) - getOpenRouterPriority(b.id)
...
function getFallbackModels(providerId: string): ModelInfo[] {
  const config = configStore.get()
  const isOpenRouter = providerId === 'openai' && config.openaiBaseUrl?.includes('openrouter.ai')
