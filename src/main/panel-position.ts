...
export type PanelPosition = "top-left" | "top-center" | "top-right" | "bottom-left" | "bottom-center" | "bottom-right" | "custom"
...
export function calculatePositionForPreset(
  ...
  switch (position) {
    ...
    case "bottom-left":
      return {
        x: screenSize.x + margin,
        y: Math.floor(screenSize.y + (screenSize.height - size.height) - margin),
      }
...
export function constrainPositionToScreen(
  ...
  constrainedPosition.x = Math.max(
    screenSize.x,
    Math.min(constrainedPosition.x, screenSize.x + screenSize.width - size.width)
  )
  
  // Ensure the panel doesn't go off the top or bottom edge
  constrainedPosition.y = Math.max(
    screenSize.y,
    Math.min(constrainedPosition.y, screenSize.y + screenSize.height - size.height)
  )
