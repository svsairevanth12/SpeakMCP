...
export const router = {
  ...
  // Test zoom functions for debugging
  "test-zoom-in": t.procedure.action(async () => {
    const win = WINDOWS.get("main") || WINDOWS.get("panel")
    if (!win) {
      throw new Error("No window found")
    }

    console.log('Test zoom in called from IPC')
    const currentLevel = win.webContents.getZoomLevel()
    const currentFactor = win.webContents.getZoomFactor()
    console.log('Before zoom in - level:', currentLevel, 'factor:', currentFactor)

    // Try multiple methods
    win.webContents.zoomIn()
    win.webContents.setZoomFactor(currentFactor * 1.2)

    setTimeout(() => {
      const newLevel = win.webContents.getZoomLevel()
      const newFactor = win.webContents.getZoomFactor()
      console.log('After zoom in - level:', newLevel, 'factor:', newFactor)
    }, 100)

    return { success: true, currentLevel, currentFactor }
  }),

  "test-zoom-out": t.procedure.action(async () => {
    const win = WINDOWS.get("main") || WINDOWS.get("panel")
    if (!win) {
      throw new Error("No window found")
    }

    console.log('Test zoom out called from IPC')
    const currentLevel = win.webContents.getZoomLevel()
    const currentFactor = win.webContents.getZoomFactor()
    console.log('Before zoom out - level:', currentLevel, 'factor:', currentFactor)
    ...
    setTimeout(() => {
      const newLevel = win.webContents.getZoomLevel()
      const newFactor = win.webContents.getZoomFactor()
      console.log('After zoom out - level:', newLevel, 'factor:', newFactor)
    }, 100)

    return { success: true, currentLevel, currentFactor }
  }),

  "test-zoom-reset": t.procedure.action(async () => {
    const win = WINDOWS.get("main") || WINDOWS.get("panel")
    if (!win) {
      throw new Error("No window found")
    }

    console.log('Test zoom reset called from IPC')
    const currentLevel = win.webContents.getZoomLevel()
    const currentFactor = win.webContents.getZoomFactor()
    console.log('Before zoom reset - level:', currentLevel, 'factor:', currentFactor)
    ...
    setTimeout(() => {
      const newLevel = win.webContents.getZoomLevel()
      const newFactor = win.webContents.getZoomFactor()
      console.log('After zoom reset - level:', newLevel, 'factor:', newFactor)
    }, 100)

    return { success: true, currentLevel, currentFactor }
  }),

  "get-zoom-info": t.procedure.action(async () => {
    const win = WINDOWS.get("main") || WINDOWS.get("panel")
    if (!win) {
      throw new Error("No window found")
    }
    ...
    console.log('Current zoom info - level:', level, 'factor:', factor)
