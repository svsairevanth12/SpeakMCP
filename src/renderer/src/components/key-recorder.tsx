...
const formatKeyCombo = (keyState: KeyState): string => {
  ...
  if (keyState.key && keyState.key !== "Control" && keyState.key !== "Shift" && keyState.key !== "Alt" && keyState.key !== "Meta") {
    // Format special keys
...
export function KeyRecorder({ value, onChange, placeholder = "Click to record keys", className, disabled }: KeyRecorderProps) {
